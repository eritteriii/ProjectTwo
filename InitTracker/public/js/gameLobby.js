			...
			for (var i = 0; i < room.characters.length; i++) {
				if (i == 0 && room.characters.length > 1) {
					$playerList.append($(
						...
						"<button type=\"button\" class=\"btn btn-dark namePlate\" draggable=\"false\">" + room.characters[i].name + ", " + room.characters[i].initiative + "</button>" +
						"</div>" +
						"<div class=\"skullbtn\">" +
						...
						"</div>" +
						"<div class=\"skullbtn\">" +
						"<button data-id='" + i + "' type=\"button\" class=\"btn btn-dark btn-circle end-turn\">" +
						"<i class=\"fas fa-angle-double-right\"></i>" +
						"</button>" +
						"</div>" +
						"<div class=\"skullbtn\">" +
						"<button data-id='" + i + "' type=\"button\" class=\"btn btn-dark btn-circle delete\">" +
			...
			$( ".delete" ).each(function(index) {
				$(this).on("click", function(){
					...
					socket.emit("PlayerLeft", {
						room: roomId, player: clickedCharacterIndex
					});
				});
			});
		} else {
...
var getMonsters = function () {
	API.getMonsters().then(function (monsters) {
		...
		for (var i = 0; i < monsters.length; i++) {
			$monsterDropdown.append($("<option></option>").attr("value", monsters[i].Type).text(monsters[i].Type));
		}
...
var saveMonster = function (event) {
	...
	if (!(monster.Type)) {
		alert("You must enter an monster Type!");
		return;
	}
